...
class UnityMultiAgentEnvWrapper:
    ...
    def __init__(self, env_file: str, train_mode: bool = False):
        ...
        self._env_info = self._env.reset(train_mode=self._train_mode)[self._brain_name]

        self.num_agents = len(self._env_info.agents)
        self.state_space_dim = self._env_info.vector_observations.shape[1]
        self.action_space_dim = self._brain.vector_action_space_size

    def print(self):
        """ Print environment information  """
        print('Number of agents:', self.num_agents)
        print('State space dim:', self.state_space_dim, ' Action space dim:', self.action_space_dim)
        print('Vector observations: ', self._env_info.vector_observations.shape)
        print(self._brain.vector_observation_space_size)
...
def train(max_episodes: int, episode_len: int = 3000):
    ...
    env = UnityMultiAgentEnvWrapper('Tennis_Linux/Tennis.x86_64', train_mode=True)
...
def cli():
    """ Deep Tennis -- command line interface """
...
def test_command(actor_weights_file: str, critic_weights_file: str):
    """ Load parameters and run the agent """
